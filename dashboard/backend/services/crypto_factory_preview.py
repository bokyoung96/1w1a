...
from dashboard.backend.schemas import (
    CategoryPointModel,
    CryptoFactoryExecutionStageModel,
    CryptoFactoryFamilyAllocationModel,
    CryptoFactoryInstrumentAllocationModel,
    CryptoFactoryPayloadModel,
    CryptoFactoryPerformanceModel,
    CryptoFactoryPerformanceSummaryModel,
    CryptoFactoryRegistryEntryModel,
    CryptoFactorySelectedStrategyModel,
    CryptoFactorySummaryModel,
)
...
class _PreviewSnapshot:
    payload: CryptoFactoryPayloadModel


class CryptoFactoryPreviewService:
    def build(self) -> CryptoFactoryPayloadModel:
        ...
        registry_entries = self._build_registry_entries(ranked, overview)
        assert report.factory_overview is not None
        return CryptoFactoryPayloadModel(
        ...
        return CandidateEvaluation(
            ...
            robustness=CandidateRobustnessMetrics(
                ...
                parameter_stability=round(
                    0.42 + (self._unit_interval(f"{candidate.candidate_id}:params") * 0.40),
                    6,
                ),
                regime_stability=round(
                    0.45 + (self._unit_interval(f"{candidate.candidate_id}:regime") * 0.42),
                    6,
                ),
            ),
        )
    ...
    def _build_paper_session(self, selected) -> PaperSession:
        ...
        for day in range(30):
            ...
            for strategy in selected:
                ...
                daily_return = strategy.returns[day]
                direction = 1.0 if strategy.signal_strength >= 0 else -1.0
                weighted_return += daily_return * direction * weight
    ...
    def _build_registry_entries(self, ranked, overview) -> list[CryptoFactoryRegistryEntryModel]:
        selected_ids = {entry.candidate_id for entry in overview.selected_basket}
        ...
        entries: list[CryptoFactoryRegistryEntryModel] = []
        for strategy in DEFAULT_STRATEGIES:
            top_score = max(scores_by_strategy.get(strategy.name, [0.0]))
            selected = any(
                candidate_id.startswith(f"{strategy.name}:")
    ...
    @staticmethod
    def _graph_points(graphs, slug: str):
        for graph in graphs:
            if graph.slug != slug:
                continue
            return [
