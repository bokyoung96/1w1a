---
source_file: "backtesting/reporting/comparison_figures.py"
type: "code"
community: "Backtesting Reporting Tests"
location: "L15"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Backtesting_Reporting_Tests
---

# ComparisonFigureBuilder

## Connections
- [[2026-04-03-performance-reporting-redesign-design]] - `rationale_for` [INFERRED]
- [[2026-04-03-performance-reporting-redesign-implementation]] - `rationale_for` [INFERRED]
- [[ComparisonFigureBuilder.__init__()]] - `contains` [EXTRACTED]
- [[ComparisonFigureBuilder._build_executive()]] - `contains` [EXTRACTED]
- [[ComparisonFigureBuilder._build_exposure()]] - `contains` [EXTRACTED]
- [[ComparisonFigureBuilder._build_performance()]] - `contains` [EXTRACTED]
- [[ComparisonFigureBuilder._build_rolling()]] - `contains` [EXTRACTED]
- [[ComparisonFigureBuilder._line()]] - `contains` [EXTRACTED]
- [[ComparisonFigureBuilder.build()]] - `contains` [EXTRACTED]
- [[ReportBuilder._build_comparison_bundle()]] - `calls` [INFERRED]
- [[comparison_figures.py]] - `contains` [EXTRACTED]
- [[test_comparison_figure_builder_writes_expected_page_assets()]] - `calls` [INFERRED]

#graphify/code #graphify/EXTRACTED #community/Backtesting_Reporting_Tests