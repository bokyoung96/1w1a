---
source_file: "tests/reporting/test_builder.py"
type: "code"
community: "Tests Reporting Test_Builder"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Tests_Reporting_Test_Builder
---

# test_builder.py

## Connections
- [[Equity Latest Quarter]] - `references` [INFERRED]
- [[Kosdaq Kosdaq Wics Sector Sector Large Cap Sector]] - `references` [INFERRED]
- [[Kospi200 1M]] - `references` [INFERRED]
- [[Map]] - `references` [INFERRED]
- [[Mkt Typ]] - `references` [INFERRED]
- [[Shares Outstanding Outstanding]] - `references` [INFERRED]
- [[Wics Sector Sector Large Cap Sector]] - `references` [INFERRED]
- [[_FakeComparisonFigureBuilder]] - `contains` [EXTRACTED]
- [[_FakeComparisonTableBuilder]] - `contains` [EXTRACTED]
- [[_FakeFactory]] - `contains` [EXTRACTED]
- [[_FakeTearsheetFigureBuilder]] - `contains` [EXTRACTED]
- [[_FakeTearsheetTableBuilder]] - `contains` [EXTRACTED]
- [[_fake_resolver()]] - `contains` [EXTRACTED]
- [[_make_plotter()]] - `contains` [EXTRACTED]
- [[_plot()]] - `contains` [EXTRACTED]
- [[_sample_run()]] - `contains` [EXTRACTED]
- [[builder.py]] - `imports_from` [EXTRACTED]
- [[models.py]] - `imports_from` [EXTRACTED]
- [[test_report_builder_creates_comparison_bundle_for_multiple_runs()]] - `contains` [EXTRACTED]
- [[test_report_builder_creates_tearsheet_bundle_and_persists_tables()]] - `contains` [EXTRACTED]
- [[test_report_builder_legacy_path_remains_available()]] - `contains` [EXTRACTED]
- [[test_report_builder_uses_universe_specific_repositories()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Tests_Reporting_Test_Builder